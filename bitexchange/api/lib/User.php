...
class User {
	...
	public static function getUserID($api_key1) {
	...
	public static function sendSMS($authy_id=false) {
		...
		curl_setopt_array($ch, array(
				...
				CURLOPT_URL => 'https://api.authy.com/protected/json/sms/'.$authy_id,
				CURLOPT_VERBOSE     => 1,
	...
	public static function sendSMSByAPI($authy_id=false) {
		global $CFG;
	...
	public static function confirmToken($token,$authy_id=false) {
		...
		if (!($token1 > 0) || !($authy_id > 0))
			...
			curl_setopt_array($ch, array(
					...
					CURLOPT_SSL_VERIFYHOST => 0,
					CURLOPT_SSL_VERIFYPEER => 0,
					CURLOPT_URL => 'https://api.authy.com/protected/json/verify/'.$token.'/'.$authy_id,
					CURLOPT_VERBOSE     => 1,
					CURLOPT_HTTPHEADER => array('X-Authy-API-Key:'.$CFG->authy_api_key)
			));
	...
	public static function userExistsWithName($first_name, $last_name) {
		
		$sql = "SELECT id FROM site_users WHERE first_name = '$first_name'";
		$result = db_query_array($sql);
	...
	public static function userExistsWithTel($tel) {
		$tel = preg_replace("/[^0-9]/", "",$tel);
	
		if (!$tel)
			return false;
	
		$sql = "SELECT id FROM site_users WHERE tel = '$tel'";
		$result = db_query_array($sql);
	...
	public static function registerNewByAPI($info) {
		global $CFG;
	
		if (!is_array($info))
			return array('error'=>array('message'=>Lang::string('account-errors-no-array'),'code'=>'INPUT_NO_ARRAY'));
		
		$info['email'] = preg_replace("/[^0-9a-zA-Z@\.\!#\$%\&\*+_\~\?\-]/", "",$info['email']);
		$exist_id = self::userExists($info['email']);
		
		//if ($exist_id > 0) {  test mail.
		if ($exist_id > 0) {
			$user_info = DB::getRecord('site_users',$exist_id,0,1);
			$email = SiteEmail::getRecord('register-existing');
			Email::send($CFG->form_email,$info['email'],$email['title'],$CFG->form_email_from,false,$email['content'],$user_info);
			return array('error'=>array('message'=>Lang::string('account-errors-duplicate-email'),'code'=>'INPUT_DUPLICATE_EMAIL'));
		}
		

		if($info['first_name'] == null || $info['last_name'] == null) {
			return array('error'=>array('message'=>Lang::string('account-errors-no-name'),'code'=>'INPUT_NO_NAME'));
		}
		
		// check duplicate
		$exist_id = self::userExistsWithName($info['first_name'], $info['last_name']);
		if ($exist_id > 0) {
			return array('error'=>array('message'=>Lang::string('account-errors-duplicate-name'),'code'=>'INPUT_DUPLICATE_NAME'));
		}
		
		if($info['pass'] == null){
			return array('error'=>array('message'=>Lang::string('account-errors-no-pass'),'code'=>'INPUT_NO_PASS'));
		}
		
		if($info['tel'] == null){
			return array('error'=>array('message'=>Lang::string('account-errors-no-tel'),'code'=>'INPUT_NO_TEL'));
		}
		
		// check duplicate
		$exist_id = self::userExistsWithTel($info['tel']);
		if ($exist_id > 0) {
			return array('error'=>array('message'=>Lang::string('account-errors-duplicate-tel'),'code'=>'INPUT_DUPLICATE_TEL'));
		}
		...
		if ($new_id > 0) {
			...
			// send token
			$query = self::registerAuthyByAPI($info['email'], $info['tel'], DEFAULT_COUNTRY); // 86 china
			$success = false;
			if($query['success'] == true) {
				$authy_id = $query['user']['id'];
				self::enableAuthyByAPI($record_id, $info['tel'], DEFAULT_COUNTRY, $authy_id, 'Y');
				$query = array('authy_id'=>$authy_id);
				$success = true;
				$result = self::sendSMSByAPI($authy_id); // send sms token
			...
			if($success == false) {
				return array('error'=>array('message'=>Lang::string('security-errors-send-sms'),'code'=>'FAILED_SEND_SMS'));
			}
			return $query;
		}
		
		return array('error'=>array('message'=>Lang::string('api-errors-db-failed'),'code'=>'DB_OPERATE_FAILED'));
	}
	
	public static function registerAuthyByAPI($email, $cell,$country_code) {
		global $CFG;
		...
		curl_setopt_array($ch, array(
				...
				CURLOPT_SSL_VERIFYHOST => 0,
				CURLOPT_SSL_VERIFYPEER => 0,
				CURLOPT_URL => 'https://api.authy.com/protected/json/users/new?api_key='.$CFG->authy_api_key,
				CURLOPT_USERAGENT => 'Codular Sample cURL Request',
				CURLOPT_POST => 1,
				CURLOPT_POSTFIELDS => array(
						'user[email]'=>$email,'user[cellphone]'=>$cell,'user[country_code]'=>$country_code
				)
		));
	...
	public static function enableAuthyByAPI($user_id, $cell,$country_code,$authy_id,$using_sms) {
		global $CFG;
		...
		return db_update('site_users',$user_id, array('tel'=>$cell,'country_code'=>$country_code,'authy_requested'=>'Y','verified_authy'=>'N','authy_id'=>$authy_id,'using_sms'=>$using_sms,'google_2fa_code'=>'','confirm_withdrawal_2fa_btc'=>'Y','confirm_withdrawal_2fa_bank'=>'Y'));
	}
	...
	public static function registerAuthy($cell,$country_code) {
		...
		curl_setopt_array($ch, array(
				...
				CURLOPT_POST => 1,
				CURLOPT_POSTFIELDS => array(
						'user[email]'=>User::$info['email'],'user[cellphone]'=>$cell,'user[country_code]'=>$country_code
				)
		));
	...
	public static function verifiedAuthyByAPI() {
		global $CFG;
		// email_2fa_verified: sms인증하기때문에 무시해버린다.
		
		if (!($CFG->session_active && $CFG->token_verified) || User::$info['verified_google'] == 'Y')
			return false;
	
		self::deleteCache();
	
		$ret = db_update('site_users',User::$info['id'],array('verified_authy'=>'Y'));
		if($ret) {
			$apikeys = APIKeys::add();
			if($apikeys != false) {
				return $apikeys;
	...
	public static function verifiedAuthy() {
		...
		if($CFG->session_active == false) {
			// trigger_error("verifyAuty session inactive", E_USER_WARNING);
			return 1;
		}
		
		if($CFG->token_verified == false) {
			// trigger_error("verifyAuty session token_verified", E_USER_WARNING);
			return 2;
		}
		
		if($CFG->email_2fa_verified == false) {
			// trigger_error("verifyAuty session email_2fa_verified", E_USER_WARNING);
			return 3;
		}
		...
		self::deleteCache();
		// trigger_error("verifyAuty ok==".User::$info['id'], E_USER_WARNING);
	...
	public static function updatePersonalInfo($info) {
		...
		if (($update['pass'] && mb_strlen($update['pass'],'utf-8') < $CFG->pass_min_chars) /*|| !$update['first_name'] || !$update['last_name'] || !$update['email']*/)
			return array('error'=>array('message'=>Lang::string('login-password-error'),'code'=>'PASSWORD_LEN_ERROR'));
