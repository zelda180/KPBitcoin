...
class Email {	
	public static function send($from,$recipients,$subject,$from_name=false,$text_version=false,$html_version=false,$variables=false) {
		...
		if(false) {
			$mail->Host = "smtp.gmail.com"; //$CFG->email_smtp_host;
			$mail->Port = "587";//$CFG->email_smtp_port;
			$mail->SMTPSecure = "tls";//$CFG->email_smtp_security;
			$mail->SMTPAuth = true;
			$mail->Username = "kaankoca97@gmail.com";//$CFG->email_smtp_username;
			$mail->Password = "test1234!";//$CFG->email_smtp_password;
			//$mail->setFrom($CFG->email_smtp_send_from,$from_name);
			$mail->setFrom("kaankoca97@gmail.com",$from_name);
		}
		else {
