...
class Orders {
	...
	public static function checkPreconditions($buy,$currency_info,$amount,$price,$stop_price,$fee,$user_available,$current_bid,$current_ask,$market_price,$user_id=false,$orig_order=false) {
		...
		if (($buy && ($total - $edit_old_fiat) > $user_available) || (!$buy && ($amount - $edit_old_btc) > $user_available)) {
			...
			return array('error'=>array('message'=>Lang::string('buy-errors-balance-too-low'),'code'=>'ORDER_BALANCE_TOO_LOW',
					'avaliable'=>$user_available, 'buy'=> ($total - $edit_old_fiat), 'sell'=>($amount - $edit_old_btc)
			));
		}

		if (($subtotal * $currency_info['usd_ask']) < $CFG->orders_min_usd) {
			return array('error'=>array('message'=>str_replace('[amount]',number_format(($CFG->orders_min_usd/$currency_info['usd_ask']),2),str_replace('[fa_symbol]',$currency_info['fa_symbol'],Lang::string('buy-errors-too-little'))),
					'code'=>'ORDER_UNDER_MINIMUM',
					'total'=>($subtotal * $currency_info['usd_ask']), 'min'=>$CFG->orders_min_usd					
			));
		}
		...
		if ($buy && !$stop_price && $price < ($current_ask - ($current_ask * (0.01 * $CFG->orders_under_market_percent))))
			return array('error'=>array('message'=>str_replace('[percent]',$CFG->orders_under_market_percent,Lang::string('buy-errors-under-market')),
					'code'=>'ORDER_TOO_FAR_UNDER_MARKET',
					'price'=>$price,
					'market'=>($current_ask - ($current_ask * (0.01 * $CFG->orders_under_market_percent)))
			));
	...
	public static function executeOrder($buy,$price,$amount,$currency1,$fee,$market_price,$edit_id=0,$this_user_id=0,$external_transaction=false,$stop_price=false,$use_maker_fee=false,$verbose=false) {
		...
		if ($error) {
			db_commit();
			//$error['error']['user_balaces'] = $user_balances;
			return $error;
		}
		...
		if ($buy) {			
			...
			if (round($amount,8,PHP_ROUND_HALF_UP) > 0) {
				if ($edit_id > 0) {
					...
					if (!$this_funds_finished) {
						if($stop_price == "") {
							db_insert('orders',array('date'=>date('Y-m-d H:i:s'),'order_type'=>$CFG->order_type_bid,'site_user'=>$this_user_id,'btc'=>$amount,'fiat'=>$amount*$price,'currency'=>$currency_info['id'],'btc_price'=>(($price != $stop_price) ? (($market_price && $max_price > 0) ? $max_price : $price) : 0),'market_price'=>(($market_price) ? 'Y' : 'N'),'log_id'=>$order_log_id,'stop_price'=>0));
						}
						else {
			...
			if (round($amount,8,PHP_ROUND_HALF_UP) > 0) {
				if ($edit_id > 0) {
					...
					if (!$this_funds_finished) {
						if($stop_price == "") {
							$insert_id = db_insert('orders',array('date'=>date('Y-m-d H:i:s'),'order_type'=>$CFG->order_type_ask,'site_user'=>$this_user_id,'btc'=>$amount,'fiat'=>($amount*$price),'currency'=>$currency_info['id'],'btc_price'=>(($price != $stop_price) ? (($market_price  && $min_price > 0) ? $min_price : $price) : 0),'market_price'=>(($market_price) ? 'Y' : 'N'),'log_id'=>$order_log_id,'stop_price'=>0));
						}
						else {
