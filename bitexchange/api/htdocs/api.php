...
$CFG->language = (!empty($_POST['lang']) && in_array(strtolower($_POST['lang']),array('en','es','ru','zh', 'kr'))) ? strtolower($_POST['lang']) : false;
...
if ($api_key1 && $api_signature1) {
	...
	elseif (!empty($result)) {
		...
		if(false) {
			$return['hash_key'] = $result[0]['secret'];
			$return['hash'] = $hash;
			$return['hash_data'] = json_encode($decoded,JSON_NUMERIC_CHECK);
			$return['signature'] = $api_signature1;
		}
