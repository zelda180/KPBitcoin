...
if (!empty($_REQUEST['step']) && $_REQUEST['step'] == 1) {
	...
	if (!is_array(Errors::$errors)) {
		...
		if (!is_array(Errors::$errors)) {
			if ($_REQUEST['method'] != 'google') {
				// trigger_error('method='.$_REQUEST['method'], E_USER_WARNING);
				if ($_REQUEST['method'] == 'sms' || $_REQUEST['method'] == 'SMS') {
						$response = User::sendSMS($authy_id);
						// trigger_error(json_encode($response), E_USER_WARNING);
...
<div class="page_title">
	<div class="container">
		...
		<div class="pagenation">
			&nbsp;<a href="index.php"><?= Lang::string('home') ?></a> <i>/</i> <a
				href="account.php"><?= Lang::string('account') ?></a> <i>/</i> <a
				href="security.php"><?= $page_title ?></a>
		</div>
	</div>
</div>
<div class="container">
	...
	<div class="content_right">
			...
			<div class="buyform">
				<div class="content">
					<h3 class="section_label">
						<span class="left"><i class="fa fa-mobile fa-2x"></i></span> <span
							class="right"><?= Lang::string('security-enter-token') ?></span>
					</h3>
							...
							<div class="one_half">
								...
								<ul class="list_empty">
									<li><input type="submit" name="submit"
										value="<?= Lang::string('security-validate') ?>"
										class="but_user" /></li>
										<? if (User::$info['using_sms'] == 'Y') { ?>
										<li><input type="submit" name="sms"
										value="<?= Lang::string('security-resend-sms') ?>"
										class="but_user" /></li>
										<? } ?>
									</ul>
							</div>
		...
		<? } elseif (User::$info['verified_authy'] == 'Y' || $step2) { ?>
			...
			<ul class="list_empty">
				<li><a href="security.php?remove=1" class="but_user"><i
						class="fa fa-times fa-lg"></i> <?= Lang::string('security-disable') ?></a></li>
			</ul>
		<? } elseif (User::$info['verified_google'] == 'Y' || $step4) { ?>
			<h2><?= $content['title'] ?></h2>
			<div class="text"><?= $content['content'] ?></div>
		...
		<? } elseif ($step1) { ?>
			...
			<? Errors::display(); ?>
			<form id="enable_tfa" action="security.php" method="POST">
				<input type="hidden" name="step" value="2" /> <input type="hidden"
					name="authcode" value="<?= urlencode($authcode1) ?>" />
				<div class="buyform">
					<div class="content">
						<h3 class="section_label">
							<span class="left"><i class="fa fa-mobile fa-2x"></i></span> <span
						...
						<div class="one_half">
							...
							<div class="param">
								...
								<input name="token" id="authy-token" type="text"
									value="<?= $token1 ?>" />
								<div class="clear"></div>
							</div>
							<div class="mar_top2"></div>
		...
		<? } elseif ($step3) { ?>
			...
			<? Errors::display(); ?>
			<form id="enable_tfa" action="security.php" method="POST">
				<input type="hidden" name="step" value="3" /> <input type="hidden"
					name="authcode" value="<?= urlencode($authcode1) ?>" />
				<div class="buyform">
					<div class="content">
						<h3 class="section_label">
							<span class="left"><i class="fa fa-mobile fa-2x"></i></span> <span
								class="right"><?= Lang::string('security-scan-qr') ?></span>
						</h3>
						<div class="clear"></div>
						<div class="one_half">
							<div class="spacer"></div>
							...
							<div class="calc">
								<img class="qrcode"
									src="includes/qrcode.php?sec=1&code=otpauth://totp/<?= $secret['label'] ?>?secret=<?= $secret['secret'] ?>" />
							</div>
							<div class="spacer"></div>
							<div class="param">
								<label for="token"><?= Lang::string('security-token') ?></label>
		...
		<? } else { ?>
			...
			<div class="mar_top2"></div>
			<div class="clear"></div>
			<form name="start_auth" id="enable_tfa" action="security.php"
				method="POST">
				<input type="hidden" name="step" value="1" /> <input type="hidden"
					id="send_sms" name="send_sms" value="" /> <input type="hidden"
					id="google_2fa" name="google_2fa" value="" />
				<div class="buyform">
					<div class="content">
						<h3 class="section_label">
							<span class="left"><i class="fa fa-mobile fa-2x"></i></span> <span
								class="right"><?= Lang::string('security-enable-two-factor') ?></span>
						</h3>
						<div class="one_half">
							<div class="spacer"></div>
							<div class="spacer"></div>
							...
							<div class="param">
								...
								<select name="method" id="method">
									<!-- <option
										<?= ($_REQUEST['method'] == 'google') ? 'selected="selected"' : false ?>
										value="google">Google Authenticator</option>
									<option
										<?= ($_REQUEST['method'] == 'authy') ? 'selected="selected"' : false ?>
										value="authy">Authy</option>  -->
									<option
										<?= ($_REQUEST['method'] == 'sms') ? 'selected="selected"' : false ?>
										value="SMS">SMS</option>
								</select>
								<div class="clear"></div>
							</div>
							<div class="param method_show" style="display">
							...
							<div class="param method_show" style="display">
								...
								<input name="cell" id="authy-cellphone" type="text"
									value="<?= $cell1 ?>" />
								<div class="clear"></div>
							</div>
							<div class="mar_top2"></div>
							<ul class="list_empty">
								<li><input type="submit" name="submit"
									value="<?= Lang::string('security-enable') ?>" class="but_user" /></li>
