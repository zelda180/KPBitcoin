...
$CFG->language = (!empty($_POST['lang'])) ? preg_replace("/[^a-z]/","",$_POST['lang']) : 'kr';
...
if ($api_key1 && (strlen($api_key1) != 16 || strlen($api_signature1) != 64)) {
	...
	$return['signature_len'] = strlen($api_signature1);
...
if($endpoint  == 'signup') {
	...
	$user_info['pass'] = $_POST['pass'];
	$user_info['email'] = $_POST['email'];
	$user_info['tel'] = $_POST['tel'];
	
	API::add('User','registerNewByAPI', array($user_info));
	$query = API::send();
	$result = $query['User']['registerNewByAPI']['results'][0];
		
	if ($result && empty($result['error'])) {
		$return['authy_id'] = $result["authy_id"];
	}
	else {
		$return['errors'][] = $result['error'];
	}
}
else if($endpoint == 'verify_user') {
	$token1 = $_POST['token'];
	$email = $_POST['email'];
	$tel = $_POST['tel'];
	$authy_id = $_POST['authy_id'];
	$pass = $_POST['pass'];
	
	// 1. email과 tel에 의해 아이디를 찾고 로그인해서 세션확보
	$login = User::logIn(0,$pass,$email);
	...
	elseif (!$login || !empty($login['error'])) {
		$is_error = true;
		$return['errors'][] = array('error'=>array('message'=>Lang::string('withdraw-account-not-found'),'code'=>'USER_NOT_EXIST'));
	}
	
	// 2. token에 의해서 verifiedAuthy를 인증하고 이함수 호출해서 인증기발 설정,  api-key를 생성해서 돌려준다.
	if($is_error == false) {
		API::token($token1);
		API::add('User','verifiedAuthyByAPI');
		$query = API::send();
		...
		if (!empty($query['error'])) {
			...
			if ($query['error'] == 'security-com-error')
				$return['errors'][] = array('error'=>array('message'=>Lang::string('security-com-error'),'code'=>'SECURUTY-COM-ERROR'));
		
			if ($query['error'] == 'authy-errors')
				$return['errors'][] = array('error'=>array('message'=>Lang::string('authy_errors'),'code'=>'AUTHY_ERORRS'));
				
			if ($query['error'] == 'request-expired')
				$return['errors'][] = array('error'=>array('message'=>Lang::string('settings-request-expired'),'code'=>'SETTINGS_REQUEST_EXPIRED'));
		}
		else {
			$result = $query['User']['verifiedAuthyByAPI']['results'][0];
			if (!empty($result['error'])) {
				$return['errors'][] = $result['error'];
			}
			else {
				$return['api_key'] = $result["key"];
				$return['api_secret_key'] = $result["secret"];
			}
		}	
	}
}
	...
	if ($post) {
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				if($is_error== false) {
					API::add('User','getInfo',array($login['session_id']));
					$query = API::send();
					$return['user'] = $query['User']['getInfo']['results'][0];
					$return['session_id'] = $session_id;
				}
				else
					$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$login['error']);
			}
			else
...
else if($endpoint == 'update_personal_info') {
		...
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				if(!$invalid_currency) {
					...
					$user_info['email'] = $_POST['email'];
					$user_info['tel'] = $_POST['tel'];
					$user_info['id'] = $_POST['user'];
					
					API::add('User','updatePersonalInfo',array($user_info));
					$query = API::send();
					$result = $query['User']['updatePersonalInfo']['results'][0];
					if (!empty($query['error']) || !empty($result['error'])) {
						$return['errors'][] =  $result['error'];
						...
						if ($query['error'] == 'security-incorrect-token')
							$return['errors'][] = array('error'=>array('message'=>Lang::string('security-incorrect-token'),'code'=>'SETTINGS_INCORRECT_TOKEN'));
					}
					else {
...
else if($endpoint == 'verify_token') { 
	$token1 = $_POST['token'];
	$verify = User::verifyToken($token1, true);
	if ($verify) {
	...
	$user_id = $_POST['user'];
	$result = User::logout(true, $user_id);
	if($result == true) {
		...
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				if(!$invalid_currency) {
					...
					API::add('User','getUserID',array($_POST['api_key']));
					$user = null;
					$query = API::send();
					if($query['User']['getUserID']['results'][0] == false) {
						$return['errors'][] = array('message'=>'Not authorized.','code'=>'AUTH_NOT_AUTHORIZED');
					}
					else {
						$user = $query['User']['getUserID']['results'][0];
					}
					
					if($user != null) {
						API::add('User','updateBalances',array($user, array($currency1=>$amount)));
						$query = API::send();
						if($query['User']['updateBalances']['results'][0] == false) {
							$return['errors'][] = array('message'=>'DB Operate error.','code'=>$query['error']);
						}
						else {
							$return['user'] = $user;
						}
...
else if($endpoint == 'get-current-bid-ask') {
	if ($post) {
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				if(!$invalid_currency) {
					...
					$currency1 = (!$currency1) ? 'krw' : strtolower($currency1);
					$currency = $CFG->currencies[strtoupper($currency1)];
...
else if($endpoint == 'get-bitcoin-address') {
	if ($post) {
		if (!$invalid_signature && $api_key1 && $nonce1 > 0) {
			if ($permissions['p_view'] == 'Y') {
				...
				if(!$invalid_currency) {
					...
					API::add('BitcoinAddresses','getAddressFromEmail',array($email));
					$query = API::send();
					$address = $query['BitcoinAddresses']['getAddressFromEmail']['results'][0];
					if($address == false) {
						$return['errors'][] = array('message'=>'User does not exist.','code'=>$query['error']);
					}
					else {
						$return['bitcoin_address'] = $address['address'];
					}
				}
				else
					$return['errors'][] = array('message'=>'Invalid authentication.','code'=>$query['error']);
				...
				elseif (strtolower($currency1) != 'btc' && !$account1)
					...
					if (strtolower($currency1) == 'btc') {
						...
						elseif ($amount1 > $user_available['BTC']) {
							$return['errors'][] = array('message'=>Lang::string('withdraw-too-much'),'code'=>'WITHDRAW_BALANCE_TOO_LOW', 'btc'=> $user_available);
							$error = true;
						}
